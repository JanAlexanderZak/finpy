...
class TestCashflow:
    ...
    def test_cashflow_and_profit(self, cash_profit, non_cash_profit, cash_expense, non_cash_expense, expected, df_cf):
        _, cf = df_cf
        assert cf.cashflow_and_profit(cash_profit, non_cash_profit, cash_expense, non_cash_expense) == expected

    def test_debt_capacity(self, df_cf):
        df, cf = df_cf
        assert cf.debt_capacity() == pytest.approx(286.6666, 3)

    def test_free_cashflow(self, df_cf):
        df, cf = df_cf
        assert cf.free_cashflow() == pytest.approx(43.3333, 3)

    def test_average_cashflow(self, df_cf):
        df, cf = df_cf
